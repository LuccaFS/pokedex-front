...
export class CommonService {
  ...
  public async getGamesDetails(gameId?: number): Promise<GameModel[]>{
      return new Promise((resolve, reject) => {
        this.http.get(`${this.api}/Detail/Games?${gameId ? `gameId=${gameId}` : ''}`)
        .subscribe(
          (response: any) => {
            resolve(response);
          },
  ...
  public async getShinyMethodsDetails(methodId?: number): Promise<ShinyMethodModel[]>{
      return new Promise((resolve, reject) => {
        this.http.get(`${this.api}/Detail/ShinyMethods?${methodId ? `shinyMethodId=${methodId}` : ''}`)
        .subscribe(
          (response: any) => {
            resolve(response);
          },
